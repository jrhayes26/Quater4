...
# === Step 1: Fetch Top Books by Category ===
def fetch_nyt_book_news():
    ...
    try:
        ...
        for list_info in data["results"]["lists"][:5]:  # First 5 categories
            category_name = list_info["display_name"]
            books = []
            for book in list_info["books"][:2]:  # Top 2 books per category
                books.append({
                    "title": book["title"],
                    "author": book["author"],
...
def format_email(categorized_books):
    ...
    <body style='font-family: Arial, sans-serif; padding: 20px;'>
        <h1 style='font-size: 26px; color: #333;'>📚 NYT Book Highlights – Curated for You</h1>
        <p style='font-size: 16px;'>Explore top picks from this week's New York Times bestseller lists.</p>
    """
...
schedule.every().monday.at("09:00").do(run_newsletter)
print("📅 Weekly newsletter scheduler started. Waiting to send every Monday at 9:00 AM...")
